...
const FlowWave = () => {
  ...
  useEffect(() => {
    ...
    const flameColor      = '#00FFA8';
    const flameColor2     = '#FF2D9E';
    const flameAmt        = 0.2;
    const atmoColor       = '#FF2D9E';
    const atmoCount       = 300;
    const atmoSize        = 24;
    const atmoSpeed       = 1.0;
    const colorLow        = '#06060F';
    const colorHigh       = '#FF2D9E';
    const opacity         = 0.26;
    const pointSize       = 5.5;
    ...
    uniform vec3 uCursor; uniform float uRepelRadius;
    uniform float uRepelStrength; uniform float uActivity;
    ...
    // motes follow camera each frame — set in animation loop:
    ...
    const FinalPassShader = {
      ...
      fragmentShader: `
        ...
        uniform vec3 uBg; uniform vec3 uFlameA;
        uniform vec3 uFlameB; uniform float uFlameAmt;
        varying vec2 vUv;
    ...
    const sz = new THREE.Vector2(W, H);
    ...
    const torusBloom = new UnrealBloomPass(sz, 0.22, 0.2, 0);
    ...
    const mainBloom = new UnrealBloomPass(sz, 0.4, 0.55, 0);
    bloomComposer.addPass(mainBloom);
    bloomComposer.addPass(new ShaderPass(GammaCorrectionShader));

    // finalPass + finalComposer
    ...
    // project cursor onto z=0 world plane
    ...
    function updateScene(scroll, m) {
      ...
      // camera fly path (first 35% of scroll)
